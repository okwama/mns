...
var SalesRepAttendanceService_1;
...
let SalesRepAttendanceService = SalesRepAttendanceService_1 = class SalesRepAttendanceService {
    dataSource;
    databaseHealthService;
    logger = new common_1.Logger(SalesRepAttendanceService_1.name);
    ...
    async getAttendance(salesRepId, from, to) {
        return this.databaseHealthService.executeWithRetry(async () => {
            try {
                ...
                const salesRepDetails = await this.dataSource.query('SELECT id, name, email, phoneNumber, country, region, route FROM SalesRep WHERE id = ? AND status = 0', [salesRepId]);
                if (!salesRepDetails || salesRepDetails.length === 0) {
                    throw new Error('Sales rep not found');
                }
                const salesRep = salesRepDetails[0];
                const attendanceHistory = await this.dataSource.query(`SELECT 
          ...
          ORDER BY dates.date DESC`, [fromDate, salesRepId, fromDate, toDate]);
    ...
    async getTodayAttendanceSummary() {
        return this.databaseHealthService.executeWithRetry(async () => {
            try {
                ...
                const totalSalesReps = await this.dataSource.query('SELECT COUNT(*) as total FROM SalesRep WHERE status = 0');
                const todaySessions = await this.dataSource.query('SELECT lh.userId, sr.name as sales_rep_name FROM LoginHistory lh JOIN SalesRep sr ON lh.userId = sr.id WHERE DATE(lh.sessionStart) = ?', [today]);
    ...
    async getAllSalesRepsWithAttendance() {
        return this.databaseHealthService.executeWithRetry(async () => {
            try {
                const today = new Date().toISOString().split('T')[0];
                const allSalesReps = await this.dataSource.query('SELECT id, name, email, phoneNumber, country, region, route FROM SalesRep WHERE status = 0');
                const todaySessions = await this.dataSource.query('SELECT DISTINCT userId FROM LoginHistory WHERE DATE(sessionStart) = ?', [today]);
                ...
                const salesRepsWithAttendance = allSalesReps.map((salesRep) => ({
                    id: salesRep.id,
                    name: salesRep.name,
                    email: salesRep.email,
                    phoneNumber: salesRep.phoneNumber,
                ...
                return {
                    ...
                    presentCount: salesRepsWithAttendance.filter((sr) => sr.isPresent).length,
                    absentCount: salesRepsWithAttendance.filter((sr) => !sr.isPresent).length,
    ...
    async getDailyAttendanceForDateRange(from, to) {
        return this.databaseHealthService.executeWithRetry(async () => {
            try {
                const defaultTo = new Date().toISOString().split('T')[0];
                ...
                const toDate = to || defaultTo;
                const allSalesReps = await this.dataSource.query('SELECT id, name, email, phoneNumber, country, region, route FROM SalesRep WHERE status = 0');
                const attendanceData = await this.dataSource.query(`SELECT 
            lh.userId,
            DATE(lh.sessionStart) as date,
            lh.sessionStart,
            lh.sessionEnd
          ...
          ORDER BY lh.sessionStart DESC`, [fromDate, toDate]);
        ...
        return this.databaseHealthService.executeWithRetry(async () => {
            try {
                ...
                return {
                    ...
                    totalSalesReps: totalSalesReps && totalSalesReps[0] ? totalSalesReps[0].total : null,
...
exports.SalesRepAttendanceService = SalesRepAttendanceService = SalesRepAttendanceService_1 = __decorate([
